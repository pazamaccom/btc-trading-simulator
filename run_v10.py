"""
	v10 Runner — Adaptive Labels + Confidence Filter + Smarter Exits
	================================================================
	Building on v9's optimized architecture (pre-computed features, hourly data, Kelly sizing):
	
	NEW in v10:
	1. ATR-SCALED ADAPTIVE LABELS: Threshold adjusts to current volatility regime
	   - Low vol → tighter threshold (captures smaller but real moves)
	   - High vol → wider threshold (avoids noise from big swings)
	2. CONFIDENCE FILTERING: Rolling accuracy tracker reduces/skips trades when
	   recent predictions are performing poorly (feedback loop)
	3. PROFIT-SCALED EXITS: Trailing stops tighten as unrealized PnL grows;
	   ATR-percentile TP/SL adapts to volatility regime
"""
import sys
...
from btc_backtester_v5 import (
    ...
    build_feature_matrix, sample_equity_curve, ML_AVAILABLE
)

print("Bitcoin Trading Simulator v10 — Adaptive Labels + Confidence + Smart Exits")
print("=" * 70)
...
# ══════════════════════════════════════════════════════
# DATA FETCHING — HOURLY (same as v9)
# ══════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════
# v10 IMPROVEMENT #1: ATR-SCALED ADAPTIVE LABELS
# ══════════════════════════════════════════════════════

def create_adaptive_labels(df, horizon=6, base_threshold=0.015, atr_period=14,
                           atr_lookback=240, blend_alpha=0.3):
    """
    Create labels where the threshold is a BLEND of fixed and ATR-adaptive.
    
    blend_alpha controls mix: 0.0 = pure fixed (like v9), 1.0 = pure adaptive.
    Default 0.3 = mostly fixed with slight adaptation. This preserves v9's
    proven label quality while adding mild volatility awareness.
    """
    close = df['close']
    atr = calc_atr(df['high'], df['low'], close, atr_period)
    
    # Rolling median ATR as the "normal" baseline
    atr_median = atr.rolling(window=atr_lookback, min_periods=atr_period * 2).median()
    atr_median = atr_median.ffill().bfill()
    
    # Scale factor: current ATR relative to its rolling median
    atr_scale = atr / atr_median
    atr_scale = atr_scale.clip(0.6, 1.8)
    
    # Pure adaptive threshold
    adaptive_threshold = base_threshold * atr_scale
    
    # Blend: mix of fixed base_threshold and adaptive
    dynamic_threshold = (1 - blend_alpha) * base_threshold + blend_alpha * adaptive_threshold
    
    # Reasonable bounds
    min_threshold = base_threshold * 0.6
    ...
    # Labels: compare future return against the dynamic threshold at signal time
    labels = pd.Series(0, index=df.index)
    labels[future_return > dynamic_threshold] = 1
    labels[future_return < -dynamic_threshold] = -1
    
    return labels, dynamic_threshold


# ══════════════════════════════════════════════════════
# v10 IMPROVEMENT #2: CONFIDENCE FILTER (TRADE ACCURACY TRACKER)
# ══════════════════════════════════════════════════════

class ConfidenceTracker:
    """
    Tracks rolling prediction accuracy and adjusts sizing/filtering.
    
    Maintains a window of recent trade outcomes. When the rolling win rate
    drops below a threshold, it signals to reduce position size or skip trades.
    """
    
    def __init__(self, window=20, min_trades=5,
                 cold_streak_threshold=0.30,
                 warm_threshold=0.40,
                 hot_streak_threshold=0.60,
                 cold_multiplier=0.3,
                 warm_multiplier=0.6,
                 hot_multiplier=1.15,
                 skip_when_frozen=True,
    ...
    def get_rolling_win_rate(self):
        if len(self.outcomes) < self.min_trades:
            return None
    ...
    def get_sizing_multiplier(self):
        ...
        if self.skip_when_frozen and wr < self.frozen_threshold:
            self.stats_log['skipped'] += 1
            return 0.0, True, 'frozen'
        
        if wr < self.cold_streak_threshold:
            self.stats_log['cold'] += 1
            return self.cold_multiplier, False, 'cold'
        
        if wr < self.warm_threshold:
            self.stats_log['warm'] += 1
            return self.warm_multiplier, False, 'warm'
        
        if wr >= self.hot_streak_threshold:
            self.stats_log['hot'] += 1
            return self.hot_multiplier, False, 'hot'
        
        self.stats_log['normal'] += 1
...
# ══════════════════════════════════════════════════════
# PRE-COMPUTE ALL FEATURES ONCE (same as v9)
# ══════════════════════════════════════════════════════

def build_v10_features(df):
    """Enhanced feature matrix for hourly data. Same as v9 + ATR percentile feature."""
    ...
    # v10: ATR percentile feature
    atr_14 = calc_atr(high, low, close, 14)
    atr_median_240 = atr_14.rolling(240, min_periods=30).median()
    features['atr_percentile'] = atr_14 / atr_median_240.replace(0, np.nan)
    features['atr_percentile'] = features['atr_percentile'].clip(0.3, 3.0)
...
t_feat = _time.time()
ALL_FEATURES = build_v10_features(df)

from btc_backtester_v5 import create_labels
ALL_LABELS_6 = create_labels(df, 6, 0.015)
ALL_LABELS_4 = create_labels(df, 4, 0.012)
ALL_LABELS_8 = create_labels(df, 8, 0.02)

_, THRESHOLDS_6 = create_adaptive_labels(df, 6, 0.015)
_, THRESHOLDS_4 = create_adaptive_labels(df, 4, 0.012)
_, THRESHOLDS_8 = create_adaptive_labels(df, 8, 0.02)
...
for name, thresh in [('h6', THRESHOLDS_6), ('h4', THRESHOLDS_4), ('h8', THRESHOLDS_8)]:
    valid = thresh.dropna()
    print(f"   {name} threshold: min={valid.min():.4f} median={valid.median():.4f} max={valid.max():.4f}")


# ══════════════════════════════════════════════════════
# KELLY POSITION SIZING (same as v9)
# ══════════════════════════════════════════════════════
...
class FastEnsembleV10:
    """RF+GB ensemble — same as v9 but used with adaptive labels."""
...
def v10_ensemble_walkforward(df, all_features, all_labels, label='Ensemble v10',
                            ...
                            trail_vol_scale=0.3,
                            profit_trail_tighten=True,
                            profit_trail_start=0.01,
                            profit_trail_max_tighten=0.50,
                            profit_trail_scale=15.0,
                            atr_adaptive_tpsl=True,
                            atr_low_percentile_sl_boost=1.2,
                            atr_high_percentile_tp_boost=1.3,
                            ...
                            confidence_min_trades=5,
                            confidence_cold_threshold=0.30,
                            confidence_warm_threshold=0.40,
                            confidence_hot_threshold=0.60,
                            confidence_cold_mult=0.3,
                            confidence_warm_mult=0.6,
                            confidence_hot_mult=1.15,
                            confidence_skip_frozen=True,
                            confidence_frozen_threshold=0.20,
    ...
    confidence_tracker = ConfidenceTracker(
        ...
        min_trades=confidence_min_trades,
        cold_streak_threshold=confidence_cold_threshold,
        warm_threshold=confidence_warm_threshold,
        hot_streak_threshold=confidence_hot_threshold,
        cold_multiplier=confidence_cold_mult,
        warm_multiplier=confidence_warm_mult,
        hot_multiplier=confidence_hot_mult,
        skip_when_frozen=confidence_skip_frozen,
        frozen_threshold=confidence_frozen_threshold
    ) if use_confidence_filter else None
    
    confidence_stats = {'skipped_frozen': 0, 'adjusted_cold': 0, 'adjusted_warm': 0, 
                        'adjusted_hot': 0, 'normal': 0}
    ...
    print(f"    Trading {total_bars} OOS bars (~{total_bars/CANDLES_PER_DAY:.0f} days) | Kelly: {use_kelly} | ConfFilter: {use_confidence_filter}")
    ...
    for i in range(start_idx, len(df)):
            ...
            if len(feat_slice) >= 200:
                try:
                    ...
                    if new_ens.train(feat_slice, label_slice):
                        ...
                        conf_info = confidence_tracker.get_stats() if confidence_tracker else {}
                        refit_log.append({'bar': i, 'date': str(today['time']),
                            'regime': regime, 'regime_conf': round(regime_conf, 2),
                            'kelly': kelly_info, 'confidence': conf_info})
                except Exception as e:
                    pass
        ...
        def compute_v10_trail(position_type, entry_price, current_price, current_atr,
                              current_vr, strength, base_trail_atr, 
                              trail_strength_scale, trail_vol_scale):
            trail_dist = base_trail_atr * current_atr
            if strength > 0.5:
            ...
            if profit_trail_tighten:
                if position_type == 'long':
                    ...
                    unrealized_pct = 0
                if unrealized_pct > profit_trail_start:
                    excess = unrealized_pct - profit_trail_start
                    tighten_frac = profit_trail_max_tighten * (1 - np.exp(-profit_trail_scale * excess))
                    trail_dist *= (1.0 - tighten_frac)
            trail_dist = max(trail_dist, 0.4 * current_atr)
            return trail_dist

        if position > 0 and position_type == 'long':
            if dynamic_trail and current_atr > 0:
                trail_dist = compute_v10_trail('long', entry_price, price, current_atr,
                                               current_vr, strength, trail_base_atr,
                                               trail_strength_scale, trail_vol_scale)
            ...
            if dynamic_trail and current_atr > 0:
                trail_dist = compute_v10_trail('short', entry_price, price, current_atr,
                                               current_vr, strength, trail_base_atr,
                                               trail_strength_scale, trail_vol_scale)
                new_ts = price + trail_dist
        ...
        if position == 0 and current_atr > 0:
            ...
            if use_confidence_filter and confidence_tracker:
                ...
                if conf_skip:
                    confidence_stats['skipped_frozen'] += 1
                elif conf_state == 'cold':
                    confidence_stats['adjusted_cold'] += 1
                elif conf_state == 'warm':
                    confidence_stats['adjusted_warm'] += 1
                elif conf_state == 'hot':
                    confidence_stats['adjusted_hot'] += 1
                else:
                    confidence_stats['normal'] += 1
            ...
            if atr_adaptive_tpsl:
                if current_atr_pct < 0.7:
                    sl_mult *= atr_low_percentile_sl_boost
                elif current_atr_pct > 1.4:
                    tp_mult *= atr_high_percentile_tp_boost
            ...
            if sig == 1 and cooldown_remaining <= 0:
                ...
                if bear_long_block and regime == 'bear' and regime_conf > 0.55:
                    ...
                    if btc_size * price > 10:
                        ...
                        if dynamic_trail:
                            init_trail = compute_v10_trail('long', price, price, current_atr,
                                                           current_vr, strength, trail_base_atr,
                                                           trail_strength_scale, trail_vol_scale)
                ...
                if regime == 'bull' and regime_conf > 0.55:
                    ...
                    if btc_size * price > 10:
                        ...
                        take_profit = price - tp_mult * current_atr
                        if dynamic_trail:
                            init_trail = compute_v10_trail('short', price, price, current_atr,
                                                           current_vr, strength, trail_base_atr,
                                                           trail_strength_scale, trail_vol_scale)
                            trailing_stop = price + init_trail
                        else:
    ...
    return {
        ...
        'v10_features': ['adaptive_atr_labels', 'confidence_filter', 'profit_scaled_exits',
                         'atr_percentile_tpsl', 'hourly_candles', 'kelly_position_sizing',
                         'regime_classifier', 'cross_asset_features']
    }
...
results = {
    ...
    'ml_available': True,
    'v10_new_features': ['adaptive_atr_labels', 'confidence_filter', 'profit_scaled_exits', 'atr_percentile_tpsl'],
    'strategies': {}
}
...
for config in ensemble_configs:
    ...
    result = v10_ensemble_walkforward(df, ALL_FEATURES, labels, label=name, 
    ...
    if result:
        ...
        print(f"    Kelly: {kf}")
        cf = result.get('confidence_final', {})
...
output_path = '/home/user/workspace/backtest_results_v10.json'
with open(output_path, 'w') as f:
    json.dump(results, f, indent=2, default=str)
print(f"\nResults saved to {output_path}")

print("\n" + "=" * 70)
print("SUMMARY — v10 OUT-OF-SAMPLE RESULTS (Adaptive Labels + Confidence + Smart Exits)")
print("=" * 70)
