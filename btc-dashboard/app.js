// ===== BTC Trading Simulator v6 Dashboard =====
...
(function () {
  ...
  document.getElementById('meta-method').textContent =
    data.ml_available ? 'Price + Alt Data + Ensemble ML' :
  ...
  (function () {
    ...
    allStrats.sort((a, b) => {
      const av = vc.v6_oos?.[a] ?? vc.v5_oos?.[a] ?? vc.v4_oos?.[a] ?? vc.v3_oos?.[a] ?? -999;
      const bv = vc.v6_oos?.[b] ?? vc.v5_oos?.[b] ?? vc.v4_oos?.[b] ?? vc.v3_oos?.[b] ?? -999;
      return bv - av;
    });
