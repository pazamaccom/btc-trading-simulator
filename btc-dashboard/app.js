// ===== BTC Trading Simulator v8 Dashboard =====
...
(function () {
  ...
  document.getElementById('meta-method').textContent =
    data.cross_asset_available ? 'Price + Alt + Cross-Asset + Ensemble ML + Futures' :
  ...
  (function () {
    ...
    allStrats.sort((a, b) => {
      const av = vc.v8_oos?.[a] ?? vc.v7_oos?.[a] ?? vc.v6_oos?.[a] ?? vc.v5_oos?.[a] ?? vc.v4_oos?.[a] ?? vc.v3_oos?.[a] ?? -999;
      const bv = vc.v8_oos?.[b] ?? vc.v7_oos?.[b] ?? vc.v6_oos?.[b] ?? vc.v5_oos?.[b] ?? vc.v4_oos?.[b] ?? vc.v3_oos?.[b] ?? -999;
      return bv - av;
    });
