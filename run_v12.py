"""
v12 Runner — Regime-Aware Unified Model + Regime-Specific Risk Management
=========================================================================
Building on v11's proven single-model architecture:

NEW in v12:
1. REGIME AS FEATURES: Regime label + confidence fed as model inputs,
   so the unified model learns regime-conditional patterns without
   fragmenting training data.
2. MEAN-REVERSION FEATURES: Bollinger position, range width, z-score
   for better sideways opportunity detection.
3. REGIME-SPECIFIC RISK MANAGEMENT: Different TP/SL/trailing/max_hold
   per regime at the trade management level.
   - Bull: wider TP, wider SL, patient holds (ride the trend)
   - Bear: moderate TP/SL, faster exits (capture the drops)
   - Sideways: tight TP/SL, quick trades (mean-reversion scalps)
4. IMPROVED SIDEWAYS TRADING: Both long and short in sideways,
   with tighter stops and smaller position sizes for range-bound trades.
5. REGIME P&L TRACKING: Per-regime performance attribution.

Key insight: v12's initial attempt with 3 separate models fragmented the
training data too much (74% sideways starved bull/bear models). This version
keeps the proven unified ensemble but makes it regime-aware through features.
"""
import sys
...
print("Bitcoin Trading Simulator v12 — Regime-Aware Unified Model")
print("=" * 70)
...
# ══════════════════════════════════════════════════════
# REGIME CLASSIFIER (v11 proven version)
# ══════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════
# FEATURE ENGINEERING — v12 ENHANCED
# ══════════════════════════════════════════════════════

def build_v12_features(df):
    """v11 features + regime features + mean-reversion features."""
    ...
    # ── v12 NEW: Mean-reversion features for sideways trading ──
    # Bollinger %B position (where price is within the bands)
    features['bb_position'] = (close - bb_lower) / (bb_upper - bb_lower)
    features['bb_bandwidth'] = bb_width

    # Range width (recent high-low range as % of price)
    ...
    # Z-score: how far is price from recent mean
    ...
    # Distance from Bollinger midpoint (mean-reversion signal)
...
t_feat = _time.time()
ALL_FEATURES = build_v12_features(df)
...
# ══════════════════════════════════════════════════════
# PRE-COMPUTE REGIME LABELS FOR ALL BARS (for features + tracking)
# ══════════════════════════════════════════════════════
...
# Vectorized regime classification (same logic as classify_regime)
...
# v12 NEW: Add regime as features to the model
...
def select_features(features, labels, max_features=40, mi_weight=0.5, imp_weight=0.5):
    ...
    # v12: Always include regime features even if not top-ranked
    for rf_name in ['regime_bull', 'regime_bear', 'regime_sideways', 'regime_confidence']:
...
print("\n6. Feature selection (v12: including regime features)...")
...
# ══════════════════════════════════════════════════════
# CONFIDENCE FILTER (from v10/v11)
# ══════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════
# KELLY POSITION SIZING (from v9/v10/v11)
# ══════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════
# v12 UNIFIED ENSEMBLE (single model, regime-aware features)
# ══════════════════════════════════════════════════════
...
class FastEnsembleV12:
    """RF + GradientBoosting + LightGBM ensemble — same as v11 but with regime features."""
    ...
    def predict_from_row(self, feature_row, regime='sideways', regime_confidence=0.5,
        ...
        # v12: Regime-adjusted thresholds (enhanced from v11)
        ...
        if self.regime_adjust:
            if regime == 'bull':
                ...
                # v12: Sideways — allow both directions more freely
                # Slightly lower thresholds since mean-reversion signals are valuable
...
# ══════════════════════════════════════════════════════
# v12 WALK-FORWARD ENGINE (regime-aware risk management)
# ══════════════════════════════════════════════════════

def v12_walkforward(df, all_features, all_labels, selected_features, label='v12',
    ...
    # v12: Track per-regime performance
    ...
    print(f"    Trading {total_bars} OOS bars (~{total_bars/CANDLES_PER_DAY:.0f} days) | Kelly: {use_kelly} | ConfFilter: {use_confidence_filter} | Regime-Aware Risk: True")
    ...
    for i in range(start_idx, len(df)):
        ...
        # v12: Regime-specific max hold and trailing ATR
        if regime == 'bull':
        ...
        if position == 0 and current_atr > 0:
            ...
            if sig == 1 and cooldown_remaining <= 0:
                ...
                # v12: Still block longs in strong bear, but allow in sideways
    ...
    return {
        ...
        'v12_features': ['regime_as_features', 'mean_reversion_features', 'regime_specific_risk',
                         'regime_specific_trailing', 'regime_pnl_tracking', 'sideways_both_directions']
    }
...
results = {
    ...
    'v12_new_features': ['regime_as_features', 'mean_reversion_features', 'regime_specific_risk',
                         'sideways_both_directions', 'regime_pnl_tracking'],
    'strategies': {}
}
...
# Three strategy configs with regime-specific risk management
configs = [
    {
        'name': 'Regime Balanced',
        'params': {
            'horizon': 6, 'threshold': 0.015,
            'rf_n': 40, 'gb_n': 40, 'lgb_n': 60,
            'rf_depth': 3, 'gb_depth': 2, 'lgb_depth': 3,
    ...
    },
    {
        ...
        'params': {
            ...
            'regime_adjust': True, 'signal_sizing': True,
            'allow_shorts': True, 'short_adx_min': 12,
            'dynamic_trail': True,
            ...
            'bull_tp_mult': 4.0, 'bull_sl_mult': 1.8,
            'bear_tp_mult': 2.0, 'bear_sl_mult': 1.3,
            'sideways_tp_mult': 1.5, 'sideways_sl_mult': 0.8,
            'bull_max_hold': 120, 'bear_max_hold': 72, 'sideways_max_hold': 36,
            'bull_trail_atr': 1.5, 'bear_trail_atr': 1.2, 'sideways_trail_atr': 0.8,
            ...
            'refit_interval': 15 * CANDLES_PER_DAY,
        },
    },
    {
        ...
        'params': {
            ...
            'bull_tp_mult': 5.5, 'bull_sl_mult': 2.5,
            'bear_tp_mult': 3.0, 'bear_sl_mult': 2.0,
            'sideways_tp_mult': 2.0, 'sideways_sl_mult': 1.2,
            'bull_max_hold': 240, 'bear_max_hold': 120, 'sideways_max_hold': 60,
            'bull_trail_atr': 2.5, 'bear_trail_atr': 2.0, 'sideways_trail_atr': 1.2,
...
for config in configs:
    ...
    result = v12_walkforward(df, ALL_FEATURES, labels, SELECTED_FEATURES,
...
output_path = '/home/user/workspace/backtest_results_v12.json'
with open(output_path, 'w') as f:
    json.dump(results, f, indent=2, default=str)
print(f"\nResults saved to {output_path}")

print("\n" + "=" * 70)
print("SUMMARY — v12 OUT-OF-SAMPLE RESULTS (3yr, Regime-Aware Unified Model)")
print("=" * 70)
...
print(f"  Architecture: Unified model with regime features + regime-specific risk management")
print("\nDone.")
