...
# ── IB Margin ──────────────────────────────────────────
# Real IB initial margin per MBT contract (from TWS, March 2026).
# Used for: Required Capital calculation, margin checks before orders.
# IB margin scales with BTC price (~28% of notional), so this is
# approximate.  For backtesting the margin is computed dynamically:
#   margin_per_contract = max(IB_MARGIN_PER_CONTRACT, price * MULTIPLIER * IB_MARGIN_PCT)
IB_MARGIN_PER_CONTRACT = 2_417  # $ initial margin per contract (from TWS)
IB_MARGIN_PCT = 0.284           # ~28.4% of notional (2417 / 8500 at BTC=$85K)

# ── Position Sizing ────────────────────────────────────
MAX_CONTRACTS = 100     # Safety cap — must allow enough for $500K exposure at low BTC prices
...
TARGET_EXPOSURE_USD = 500_000   # Target notional exposure per trade ($)
