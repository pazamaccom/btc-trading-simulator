...
function updateBacktestView(data) {
  ...
  if (Array.isArray(regimeSummary)) {
    ...
    for (const t of trades) {
      ...
      if (!byRegime[r]) byRegime[r] = { regime: r, trades: 0, wins: 0, pnl: 0, periods: 0, bars: 0 };
      if (t.action === 'SELL' || t.action === 'COVER') {
        byRegime[r].trades++;
        byRegime[r].pnl += t.net_pnl || 0;
        if ((t.net_pnl || 0) >= 0) byRegime[r].wins++;
      }
    }
    // Count periods
  ...
  if (summaryData && summaryData.length > 0) {
    ...
    for (const s of summaryData) {
      ...
      const winCount = s.wins ?? s.winning_trades ?? 0;
      const wr = totalTrades > 0 ? ((winCount / totalTrades) * 100).toFixed(1) : '0.0';
      const totalPnl = s.pnl ?? s.total_pnl ?? 0;
  ...
  if (regimes.length > 0) {
    ...
    regimes.forEach((rp, i) => {
      ...
      const durationBars = rp.bars || rp.duration_bars || rp.duration || '—';
      ...
      if (startPrice && endPrice && startPrice > 0) {
        ...
        ret = `<span class="${colorClass(retPct)}">${retPct >= 0 ? '+' : ''}${retPct.toFixed(2)}%</span>`;
      }
  ...
  if (allTrades.length > 0) {
    ...
    btTbody.innerHTML = '<tr><td colspan="8" class="empty-state">No backtest trades</td></tr>';
  }
}
